...
; @LastEditTime: 2026-06-26 15:21:59
; @FilePath: \asm_matrix_benchmark\src\assembly\windows\float\random_matrix_float.asm
; @Description:  random matrix float nasm code on windows
; -------------------------------------------------------------
...
section .rodata
    ...
    rand_max       dd  32767.0                          ; RAND_MAX as float
...
random_matrix_float:
    ...
    ; store on stack: [rsp + 32] = max (float), [rsp + 36] = min (float)
    cmp r13, 0
    je case_size_zero
    cmp r13, 1
    je case_size_one
...
case_size_one:
    movss xmm0, [r12]                   ; xmm0 = range[0]
    xorps xmm1, xmm1                    ; xmm1 = 0.0
    comiss xmm0, xmm1
    jbe range_le_zero                   ; if range[0] <= 0

    ; range[0] > 0
    movss [rsp + 32], xmm0              ; max = range[0]
    mov dword[rsp + 36], 0              ; min = 0.0f
    jmp boundary_done

range_le_zero:
    comiss xmm0, xmm1
    jb range_neg                        ; if range[0] < 0
...
range_neg:
    ; range[0] < 0
    mov dword[rsp + 32], 0              ; max = 0.0f
    movss [rsp + 36], xmm0              ; min = range[0]
    jmp boundary_done

case_size_else:
    movss xmm0, [r12]                   ; xmm0 = range[0]
    movss xmm1, [r12 + 4]               ; xmm1 = range[1]
    comiss xmm0, xmm1
    jae first_bigger
...
boundary_done:
    ...
    movss xmm14, [rsp + 32]             ; xmm14 = max_boundary
    movss xmm13, [rsp + 36]             ; xmm13 = min_boundary
    subss xmm14, xmm13                  ; xmm14 = max - min
    movss xmm12, [rel rand_max]         ; xmm12 = RAND_MAX
...
fill_loop:
    ...
    movss [r15 + r14 * 4], xmm0         ; res->data[i] = value
    inc r14
    jmp fill_loop
