...
; @LastEditTime: 2026-06-26 15:21:55
; @FilePath: \asm_matrix_benchmark\src\assembly\windows\float\mul_matrix_float.asm
; @Description:  mul matrix float nasm code on windows
; -------------------------------------------------------------
...
loop1:
    ...
    loop2:
        ...
        loop3:
            ...
            add r8, r13                 ; r8 += k
            movss xmm1, [r14 + r8 * 4]  ; xmm1 = m1->data[i * m1->cols + k]

            ; calc second number
            mov r8, r13                 ; r8 = k
            imul r8, rcx                ; r8 *= m2->cols
            add r8, rsi                 ; r8 += j
            mulss xmm1, [r15 + r8 * 4]  ; xmm1 *= m2->data[k * m1->cols + j]
    ...
    give_value:
        ...
        add r8, rsi                     ; r8 += j
        movss [r11 + r8 * 4], xmm0      ; res->data[i * res->cols + j] = sum
        inc rsi                         ; j++
        jmp loop2
