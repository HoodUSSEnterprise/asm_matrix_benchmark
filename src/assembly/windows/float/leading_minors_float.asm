...
; @LastEditTime: 2026-06-26 15:21:50
; @FilePath: \asm_matrix_benchmark\src\assembly\windows\float\leading_minors_float.asm
...
get_leading_minors_float:
    ...
    mov rcx, 16                         ; sizeof(Leading_Minors_Float)
    call malloc
    test rax, rax
    ...
    ; m->rows * sizeof(MatrixFloat) = m->rows * 24
    mov rcx, r8
    imul rcx, 24
    call malloc
    test rax, rax
...
order_loop:
    ...
    copy_loop_i:
        ...
        copy_loop_j:
            ...
            movss xmm0, [r13 + rax * 4] ; xmm0 = m->data[i * m->cols + j]
