...
; @LastEditTime: 2026-06-26 15:21:57
; @FilePath: \asm_matrix_benchmark\src\assembly\windows\float\print_matrix_float.asm
...
loop2:
    ...
    movss xmm1, [r15 + r11 * 4]
    cvtss2sd xmm1, xmm1                 ; float to double for printf %f
    movq rdx, xmm1                      ; set rdx for shadow space (required by Windows x64 variadic)
    mov al, 1                           ; set AL = number of FP arguments for variadic function
    call printf                         ; printf("%f ", matrix->data[i * matrix->cols + j]);
    inc r14                             ; j++
    jmp loop2
