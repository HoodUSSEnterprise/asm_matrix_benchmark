...
; @LastEditTime: 2026-06-26 15:21:39
; @FilePath: \asm_matrix_benchmark\src\assembly\windows\float\extract_col_matrix_float.asm
; @Description:  extract col matrix float nasm code on windows
; -------------------------------------------------------------
...
loop1:
    ...
    movss xmm0, [r13 + r8 * 4]          ; xmm0 = m->data[i * m->cols + index]
    movss [r12 + rsi * 4], xmm0         ; res->data[i] = m->data[i * m->cols + index]
