...
; @FilePath: \asm_matrix_benchmark\src\assembly\windows\double\determinant_matrix_double.asm
; @Description: determinant matrix double nasm code on windows
; -------------------------------------------------------------

global determinant_double
extern puts
extern malloc
extern free
...
determinant_double:
    ...
    ; malloc new data (double array copy)
    mov rdi, r8                         ; rdi = m->rows
    imul rdi, r9                        ; rdi = m->rows * m->cols
    mov rcx, rdi                        ; rcx = len of array
    shl rcx, 3                          ; rcx *= 8 (sizeof double)
    call malloc
    test rax, rax
    jz malloc_fail_data

    ; save malloc res data
    mov rbx, rax                        ; rbx = data (double*)
    mov r12, [r14]                      ; r12 = m->data (double*)

    ; copy data
    mov rcx, rdi                        ; rcx = element count
    shl rcx, 3                          ; rcx = byte count
    mov rdi, rbx                        ; rdi = dest
    mov rsi, r12                        ; rsi = src
    rep movsb                           ; memcpy(data, m->data, bytes)

next:
    ; Gaussian elimination with row swap sign tracking
    mov r12, rbx                        ; r12 = data (double*)
    mov r8, [r14 + 8]                   ; r8 = m->rows
    mov r9, [r14 + 16]                  ; r9 = m->cols
    xor rcx, rcx                        ; rows = 0
    xor rdx, rdx                        ; cols = 0
    mov esi, 1                          ; sign = 1 (track row swaps)
...
loop1:
    ...
    ; find main element
    mov r11, rcx                        ; r11 = pivot = rows

    find_pivot:
        cmp r11, r8                     ; pivot < m->rows
        jge col_zero
    ...
    col_zero:
        ; this column is all zeros → determinant = 0
        movsd xmm0, [rel epsilon]
        subsd xmm0, xmm0                ; xmm0 = 0.0
        movsd [r15], xmm0               ; *det = 0.0
        jmp end
    ...
    pivot_found:
        ...
        ; swap rows: pivot <-> rows, flip sign
        xor r13, r13                    ; j = 0
        neg esi                         ; sign = -sign
...
calc_det:
    ; multiply diagonal elements, apply sign
...
det_mul_loop:
    ...
    mulsd xmm0, xmm1                    ; product *= data[i][i]
...
det_done:
    ...
    cmp esi, 1
    je store_result
    mov rax, 0x8000000000000000         ; sign bit for double
    movq xmm1, rax
    xorpd xmm0, xmm1                    ; flip sign

store_result:
    movsd [r15], xmm0                   ; *det = result
    jmp end
