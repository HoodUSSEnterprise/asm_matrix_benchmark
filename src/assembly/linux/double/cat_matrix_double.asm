...
; @LastEditTime: 2026-06-26 12:13:25
; @FilePath: \asm_matrix_benchmark\src\assembly\linux\double\cat_matrix_double.asm
; @Description: cat matrix double nasm code on linux
; -------------------------------------------------------------
...
cat_matrix_double:
    ...
    ; malloc new res
    mov rdi, 24                         ; sizeof(MatrixDouble) = 24
    call malloc wrt ..plt
    test rax, rax
...
loop1:
    ...
    matrix_after1:
        ...
        add r9, rdx                     ; r9 += j
        movsd xmm0, [r15 + r9 * 8]      ; xmm0 = m2->data[(i - m1->rows) * res->cols + j]
