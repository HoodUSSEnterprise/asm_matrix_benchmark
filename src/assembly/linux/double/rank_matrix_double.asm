...
; @Date: 2026-06-24 20:34:12
; @LastEditors: HoodUSSEnterprise
; @LastEditTime: 2026-06-26 15:15:49
; @FilePath: \asm_matrix_benchmark\src\assembly\linux\double\rank_matrix_double.asm
; @Description: rank matrix double nasm code on linux
; -------------------------------------------------------------

global rank_matrix_double
extern puts
extern malloc
extern free
...
rank_matrix_double:
    ...
    mov rbx, rax                        ; rbx = new data (double*)
    mov r13, [r14]                      ; r13 = m->data (double*)

    xor rdx, rdx                        ; i = 0
    mov rdi, r12                        ; restore count

loop_copy:
    cmp rdx, rdi                        ; i < m->rows * m->cols
    jge next

    movsd xmm0, [r13 + rdx * 8]         ; xmm0 = m->data[i]
    movsd [rbx + rdx * 8], xmm0         ; new_data[i] = m->data[i]
...
loop_calc_rank1:
    ...
    loop_calc_rank2:
        ...
        movsd xmm2, [rel epsilon]       ; xmm2 = 1e-6
        comisd xmm0, xmm2
        jnb is_not_zero                 ; if fabs(value) >= 1e-6, this row is non-zero
        inc rdx                         ; j++
        jmp loop_calc_rank2
