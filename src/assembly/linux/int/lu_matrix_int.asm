...
; @LastEditTime: 2026-06-26 15:19:49
; @FilePath: \asm_matrix_benchmark\src\assembly\linux\int\lu_matrix_int.asm
; @Description:lu decomposition nasm code on linux 
; -------------------------------------------------------------

global LU_Decomposition_int
extern printf
extern puts
extern malloc
extern free
extern get_leading_minors_int
...
section .rodata
    ...
    wrong_lm       db  "function get_leading_minors_int has a wrong value", 0
...
LU_Decomposition_int:
    ...
    ; ========== get leading minors ==========
    mov rdi, r14
    call get_leading_minors_int wrt ..plt
    test rax, rax
...
check_lm_done:
    ...
    mov r13, rax                        ; r13 = element count (callee-saved)
    shl rax, 3                          ; rax *= 8
    mov rdi, rax
    call malloc wrt ..plt
    test rax, rax
...
init_U_first_row:
    ...
    movsxd rax, [r9 + rsi * 4]          ; m->data[0 * n + j] = m->data[j]
    cvtsi2sd xmm0, rax
...
init_U_done:
    ...
    ; u_00 is U->data[0] (first element)
...
compute_l_i0:
    ...
    mov rax, rdi
    imul rax, r8                        ; rax = i * n
    movsxd rax, [r9 + rax * 4]          ; m->data[i * n] sign-extended
    cvtsi2sd xmm0, rax
...
doolittle_done:
    ...
    ; ========== free leading_minors ==========
    ; free matrix_data[i].data for each i
    xor rcx, rcx                        ; i = 0
