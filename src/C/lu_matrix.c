...
@LastEditTime: 2026-06-24 19:02:46
...
bool LU_Decomposition_float(MatrixFloat *m, LU_Result *res)
{
    ...
    // check the matrix can lu decomposition or not
    Leading_Minors_Float *leading_minors = get_leading_minors_float(m);
    if (leading_minors == NULL)
    {
        fprintf(stderr, "function get_leading_minors_float has a wrong value\n");
        return NULL;
    }
    for (size_t i = 0; i < leading_minors->len; i++)
    {
        int rank = 0;
        if (rank_matrix_float(leading_minors->matrix_data + i, &rank) == false)
        {
            return false;
        }
        else
    ...
    */
    for (size_t i = 0; i < U->cols; i++)
    {
        U->data[i] = (double)m->data[i];
    }
    for (size_t i = 1; i < L->rows; i++)
    {
        L->data[i * L->cols + 0] = (double)m->data[i * m->cols + 0] / U->data[0 * U->cols + 0];
    }
    ...
    */
    for (size_t i = 1; i < L->rows; i++)
    {
        for (size_t j = i; j < L->rows; j++)
        {
            ...
            U->data[i * U->cols + j] = (double)m->data[i * m->cols + j] - sum;
        }
        for (size_t j = i + 1; j < L->rows; j++)
        {
            double sum = 0;
            ...
            L->data[j * L->cols + i] = ((double)m->data[j * m->cols + i] - sum) / U->data[i * U->cols + i];
        }
    }
    // final result
    res->L = L;
...
bool LU_Decomposition_double(MatrixDouble *m, LU_Result *res)
{
    ...
    // check the matrix can lu decomposition or not
    Leading_Minors_Double *leading_minors = get_leading_minors_double(m);
    if (leading_minors == NULL)
    {
        fprintf(stderr, "function get_leading_minors_double has a wrong value\n");
        return NULL;
    }
    for (size_t i = 0; i < leading_minors->len; i++)
    {
        int rank = 0;
        if (rank_matrix_double(leading_minors->matrix_data + i, &rank) == false)
        {
            return false;
        }
        else
