...
@LastEditTime: 2026-06-24 16:16:23
...
bool rank_matrix_float(MatrixFloat *m, int *rank)
{
    ...
    // malloc new data
    float *data = (float *)malloc(sizeof(float) * m->rows * m->cols);
    if (data == NULL)
    {
        fprintf(stderr, "Memory allocation failed\n");
        return false;
    ...
    for (size_t rows = 0, cols = 0; rows < m->rows && cols < m->cols; cols++)
    {
        // find main element
        size_t pivot = rows;
        while (fabsf(data[pivot * m->cols + cols]) < 1e-5f && pivot < m->rows)
        {
            pivot++;
        }
        // that means this column is zero of all
        ...
        if (pivot != rows)
        {
            for (size_t j = 0; j < m->cols; j++)
            {
                float temp = data[pivot * m->cols + j];
                data[pivot * m->cols + j] = data[rows * m->cols + j];
                data[rows * m->cols + j] = temp;
            }
        }
        ...
        for (size_t i = rows + 1; i < m->rows; i++)
        {
            float factor = data[i * m->cols + cols] / data[rows * m->cols + cols];
            for (size_t j = cols; j < m->cols; j++)
            {
                data[i * m->cols + j] -= factor * data[rows * m->cols + j];
            }
    ...
    for (size_t i = 0; i < m->rows; i++)
    {
        ...
        for (size_t j = 0; j < m->cols; j++)
        {
            if (fabsf(data[i * m->rows + j]) >= 1e-5f)
            {
                flag = true;
                break;
            }
