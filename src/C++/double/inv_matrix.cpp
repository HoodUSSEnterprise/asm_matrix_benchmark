...
@LastEditTime: 2026-06-26 08:50:49
@FilePath: \asm_matrix_benchmark\src\C++\double\inv_matrix.cpp
...
@return {*}
*************************************************************/
MatrixDouble MatrixDouble::inv_matrix() const
{
    if (rows != cols)
    {
    ...
    {
        mat[i].resize(cols * 2, 0.0);
        for (size_t j = 0; j < cols; j++)
        {
            mat[i][j] = data[i * cols + j];
        }
        // Set identity on the right half
        mat[i][cols + i] = 1.0;
    }
    ...
    // NOTE: This implementation performs only forward elimination.
    // A full Gauss-Jordan elimination (back-substitution) is needed
    // for a truly accurate inverse.
